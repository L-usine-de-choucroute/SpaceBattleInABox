...
func _on_space_layer_cell_clicked(layer_level: int, cell: Vector2i):
	if (selected_tile == null):
		...
		var isMoved = try_move_ship_at_selected_tile(layer_level, cell)
		if (isMoved): unselect_tile()
...
func try_move_ship_at_selected_tile(target_layer: int, target_cell: Vector2i):
	var selected_ship = get_ship_at_selected_tile()
	if (selected_ship == null): return false
	return try_move_ship(selected_ship, selected_tile, target_layer, target_cell)
...
func add_ship(ship: Ship, target_layer: int, target_cell: Vector2i):
	try_move_ship(ship, null, target_layer, target_cell)

func try_move_ship(ship: Ship, from, target_layer: int, target_cell: Vector2i):
	var target_content = ships_location.get_ship(target_cell.x, target_cell.y, target_layer)
	if (target_content != null): return false # Prevent override another ship
